...
// ─── Helper: detection image URL (local-first, BKC signed-URL optional) ──
// Default (any self-hosted company): serve the file from IMAGE_SHARED_DRIVE through
// this API. If BKC_IMAGE_API_KEY is set, use BKC's public signed URL instead.
// image_path (the full path Python wrote) must resolve *inside* IMAGE_SHARED_DRIVE —
// this is the only guard against path traversal when touching the local filesystem.
function localImageFile(imagePath) {
  const drive = process.env.IMAGE_SHARED_DRIVE
  if (!drive || !imagePath) return null
  const resolved = path.resolve(imagePath)
  if (!resolved.startsWith(path.resolve(drive))) return null
  return fs.existsSync(resolved) ? resolved : null
}

async function resolveEventImageUrl(row) {
  if (process.env.BKC_IMAGE_API_KEY) {
    return getBkcSignedUrl(row.image_path, row.image_name)
  }
  // relative URL — frontend fetches it with the bearer token (see api.js authImage)
...
// resolve the camera's company_code + last_snapshot_at from DB before building any fs path
...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    // Single-tenant: every deployment serves exactly one company. company_code
    // comes from the JWT — the server never trusts a client-supplied company.
...
app.post('/api/users', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { username, full_name, role_id, temp_password } = req.body || {}
    if (!username || !role_id) {
      return res.status(400).json({ error: 'username and role_id are required' })
    }

    // Single-tenant: every user belongs to this deployment's one company
    const company_code = req.companyCode
...
app.post('/api/users/:id/update', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (target.company_code !== req.user.company_code) {
      return res.status(403).json({ error: 'Forbidden' })
    }
...
app.get('/api/events', requireAuth, async (req, res) => {
  try {
    ...
    // ponytail: page_size is capped at 50, so N image lookups (BKC call or local
    // fs.existsSync) per list load is fine — revisit with a cache if ever unpaginated
    const data = await Promise.all(rows.map(async (row) => ({
      ...row,
      image_url: await resolveEventImageUrl(row),
    })))
    res.json({
      data,
...
app.get('/api/events/:id', requireAuth, async (req, res) => {
  try {
    ...
    // local-serving by default; BKC signed URL only if BKC_IMAGE_API_KEY is set; null → placeholder
    const image_url = await resolveEventImageUrl(ev)
    res.json({
      ...ev,
      image_url,
...
// Streams the detection image from IMAGE_SHARED_DRIVE (local-serving mode, i.e.
// BKC_IMAGE_API_KEY not set). Bearer-token protected like every other route, so the
// frontend fetches it as a blob (see api.js authImage) rather than a bare <img src>.
app.get('/api/events/:id/image', requireAuth, async (req, res) => {
  try {
    const result = await execSP('smg.sp_get_detection_event_detail', [
      { name: 'event_id',     type: sql.BigInt,       value: parseInt(req.params.id) },
      { name: 'company_code', type: sql.NVarChar(20), value: req.companyCode },
...
app.post('/api/cameras/:camera_no/update', requireAuth, async (req, res) => {
  try {
    ...
    // 1. Fetch existing camera settings (company_code comes from the JWT — single tenant)
...
async function runMigrations() {
  try {
    ...
    console.log('Migration: Login lockout objects updated successfully')
  } catch (err) {
