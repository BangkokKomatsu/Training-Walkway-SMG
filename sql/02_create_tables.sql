...
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_role')
CREATE TABLE ww.mst_role (
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_company')
CREATE TABLE ww.mst_company (
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_user')
CREATE TABLE ww.mst_user (
    ...
    CONSTRAINT UQ_mst_user_username UNIQUE (company_code, username),
    CONSTRAINT FK_mst_user_company FOREIGN KEY (company_code) REFERENCES ww.mst_company(company_code),
    CONSTRAINT FK_mst_user_role    FOREIGN KEY (role_id)       REFERENCES ww.mst_role(role_id)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_camera')
CREATE TABLE ww.mst_camera (
    company_code    NVARCHAR(20)    NOT NULL,
    ...
    CONSTRAINT FK_mst_camera_company FOREIGN KEY (company_code) REFERENCES ww.mst_company(company_code)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_detection_area')
CREATE TABLE ww.mst_detection_area (
    ...
    CONSTRAINT FK_mst_area_camera FOREIGN KEY (company_code, camera_no)
        REFERENCES ww.mst_camera(company_code, camera_no)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'mst_config')
CREATE TABLE ww.mst_config (
    company_code    NVARCHAR(20)    NOT NULL,
    ...
    CONSTRAINT FK_mst_config_company FOREIGN KEY (company_code) REFERENCES ww.mst_company(company_code)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'trn_detection_event')
CREATE TABLE ww.trn_detection_event (
    ...
    CONSTRAINT FK_event_company FOREIGN KEY (company_code) REFERENCES ww.mst_company(company_code)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'trn_alert_log')
CREATE TABLE ww.trn_alert_log (
    ...
    CONSTRAINT FK_alert_event FOREIGN KEY (event_id) REFERENCES ww.trn_detection_event(event_id)
);
GO
...
-- -----------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
               WHERE s.name = 'ww' AND t.name = 'trn_system_log')
CREATE TABLE ww.trn_system_log (
    log_id          BIGINT          NOT NULL IDENTITY(1,1),
    company_code    NVARCHAR(20)    NOT NULL,
