...
function DetailRow({ icon: Icon, label, value, mono = false }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-border/60">
      <div className="flex items-center gap-2 text-ink-muted text-[11px] font-semibold uppercase">
        <Icon size={12} className="text-ink-subtle" />
        <span>{label}</span>
      </div>
      <span
        className={`text-xs font-bold text-ink ${mono ? 'font-mono' : ''}`}
      >
        {value ?? '—'}
      </span>
    </div>
  ...
  if (error || !ev) {
    return (
      <div className="py-20 text-center max-w-sm mx-auto">
        ...
        <p className="text-sm font-bold text-ink mb-3">{error || 'Event not found'}</p>
        <Link
          to="/events"
    ...
    <div className="max-w-[1360px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-3">
          ...
          />
          {ev.image_path && (
            <div className="p-3 rounded-lg border border-border bg-surface-2/40 text-[10px] text-ink-subtle font-mono break-all">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface/40 dark:bg-surface/20 p-5 shadow-xs">
            <h3 className="text-xs font-bold text-ink uppercase tracking-wider mb-3">Incident Metadata</h3>
              ...
              {hasStatusReport ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-xs space-y-3">
                    <div>
                      <span className="text-[10px] text-ink-subtle uppercase tracking-wider font-bold block mb-1">Closed By:</span>
                    ...
                    <div>
                      <span className="text-[10px] text-ink-subtle uppercase tracking-wider font-bold block mb-1">Closed Timestamp:</span>
                      <span className="font-mono text-ink-muted font-medium">{formatDateTime(ev.resolved_at)}</span>
                    </div>

                    <div>
                      <span className="text-[10px] text-ink-subtle uppercase tracking-wider font-bold block mb-1">Resolution Details:</span>
                ...
                <form onSubmit={e => e.preventDefault()} className="space-y-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ink-muted uppercase">Closed By (Operator):</label>
                    <input
                      type="text"
                      required
                      value={closedBy}
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-border bg-surface-2 text-ink text-xs focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all shadow-xs"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-ink-muted uppercase">Attach Verification Photo (Optional):</label>
                    <div className="relative border border-dashed border-border/80 hover:border-primary rounded-xl p-4 bg-surface-2/30 hover:bg-surface-2/50 transition-all text-center flex flex-col items-center justify-center cursor-pointer">
                      ...
                      {closeImage ? (
                        ...
                        <>
                          ...
                          <span className="text-[10px] font-bold text-ink-muted">Drag or click to attach image</span>
                          <span className="text-[9px] text-ink-subtle">PNG, JPG, or WEBP</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  {submitError && (
                    <div className="text-[10px] text-rose-500 font-bold p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/20">
                      {submitError}
                    </div>
                  )}
            ...
            <div className="rounded-xl border border-border bg-surface/40 dark:bg-surface/20 p-5 shadow-xs no-print">
              ...
              <div className="grid grid-cols-2 gap-3 mb-2">
                <div className="p-3 rounded-lg border border-border bg-surface/50 dark:bg-surface/30 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-ink-muted">MS Teams:</span>
                  <StatusBadge status={ev.teams_sent ? 'ok' : 'error'} label={ev.teams_sent ? 'Success' : 'Failed'} />
                </div>
                <div className="p-3 rounded-lg border border-border bg-surface/50 dark:bg-surface/30 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-ink-muted">Email Alert:</span>
              ...
              {ev.alert_error && (
                <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 flex gap-2">
                  ...
                  <div className="text-[10px] font-medium text-rose-500 leading-normal">
      ...
      {showFullResImage && (
        ...
        >
          <div className="relative max-w-3xl max-h-[85vh] rounded-xl overflow-hidden border border-zinc-800 bg-zinc-950 p-1 flex items-center justify-center">
            ...
            />
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 px-3 py-1.5 rounded-full text-[10px] font-bold text-white uppercase pointer-events-none">
