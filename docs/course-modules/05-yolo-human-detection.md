...
- กรองเฉพาะ class `person` (class id = 0)
...
| ชื่อ | ขนาด | ความเร็ว (CPU) | ความแม่นยำ |
|------|------|----------------|-----------|
| yolo11n | 2.6 MB | เร็วมาก | ปานกลาง |
| yolo11s | 9.4 MB | เร็ว | ดี |
| yolo11m | 20 MB | ปานกลาง | ดีมาก |
| yolo11l | 25 MB | ช้า | สูง |
| yolo11x | 56 MB | ช้ามาก | สูงสุด |
...
รับ frame จาก CameraReader
        ↓
model.predict(frame, conf=0.5, classes=[0])   ← [0] = person เท่านั้น
        ↓
...
```text
❌ ห้าม commit:
├── Models/yolo11n.pt     (ขนาดใหญ่ ~6 MB, ดาวน์โหลดเองได้)
